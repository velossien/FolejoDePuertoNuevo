...
class Main extends Component {
  ...
  render() {
    ...
    if (this.state.showHeader) {
      const isNotOnGameArt =
        this.props.location.pathname.indexOf("gameart") === -1;
      banner = (
        <Banner
          headerVisible={this.state.showHeader}
          isNotOnGameArt={isNotOnGameArt}
        />
      );
      menubar = isNotOnGameArt ? (
        <Menubar headerVisible={this.state.showHeader} />
      ) : null;
    } else {
      banner, (menubar = null);
    }
    ...
    return (
      <div className={this.state.newClassName}>
        ...
        {menubar}
        <Switch>
          <Route exact path="/" render={props => <Redirect to="/Gallery" />} />
          <Route
            exact
            path="/Gallery"
            render={props => (
